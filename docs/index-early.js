import { theTrickToViewportUnitsOnMobile } from "/js/utils.js?v=1.0.3-beta.1"
...
function calcItemWidth() {
    ...
    let listItemsPerRow = 1 + Math.floor(containerWidth / 360)
    let listItemsWidth = (containerWidth - (listItemsPerRow - 1) * gutter_size) / listItemsPerRow
    if (1 < listItemsPerRow && listItemsWidth < 240) {
        listItemsPerRow--
        listItemsWidth = (containerWidth - (listItemsPerRow - 1) * gutter_size) / listItemsPerRow
    }
    ...
    document.documentElement.style.setProperty('--lipc', `${1 + Math.floor(44 / listItemsPerRow)}`)
    ...
    window.dispatchEvent(new Event('newItemWidth'));
}
